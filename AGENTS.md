...
- `@plasius/gpu-fluid` may expose local hooks and typed planning payloads, but analytics delivery ownership stays outside this package.
...
- Active rule: package-identity, legal, and contributor-facing text in this repository must refer to `@plasius/gpu-fluid`.
...
### Fluid Package Scope Clarification (Supersedes Earlier Package-Specific Scope Notes)
- Prior decision reference: the initial package-specific bullet list at the top of this file.
- Active rule: this repository is `@plasius/gpu-fluid`, not `@plasius/gpu-performance`.
- Active rule: package-specific architecture in this repository must center fluid continuity, stable physics snapshot handoff, representation bands, and worker/performance integration for fluid preparation.
- Supersession scope: this clarifies package-specific scope only. The cross-package lifecycle, quality, and governance rules remain active.

### Fluid Analytics and Debug Policy
- `@plasius/gpu-fluid` must not invent package-local analytics delivery.
- If fluid diagnostics need export beyond local hooks, route them through `@plasius/analytics`.
- If fluid diagnostics need runtime debug surfaces, prefer integration with `@plasius/gpu-debug` rather than bespoke package-local debug clients.

### Fluid Continuity Policy
- Near, mid, far, and horizon fluid representations must preserve shared wave identity across range transitions.
- Reduced distant fidelity is acceptable; fluid disappearance or uncorrelated range-band motion is not.
